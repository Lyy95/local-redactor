...
class RuleDefinition:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            ...
            "applies_to",
            tuple(
                dict.fromkeys(scope.strip().casefold() for scope in self.applies_to if scope.strip())
            ),
        )
        self._validate()
    ...
    def from_dict(cls, value: Mapping[str, object]) -> RuleDefinition:
        try:
            return cls(
                ...
                match_mode=MatchMode(
                    _strict_string(value["match_mode"], "match_mode")
                ),
    ...
    def _validate(self) -> None:
        ...
        if not self.name or len(self.name) > MAX_RULE_NAME_LENGTH:
            raise RuleValidationError(
                f"规则名称不能为空且不能超过 {MAX_RULE_NAME_LENGTH} 个字符"
            )
        ...
        for pattern in self.patterns:
            if len(pattern) > MAX_PATTERN_LENGTH:
                raise RuleValidationError(
                    f"关键词或表达式不能超过 {MAX_PATTERN_LENGTH} 个字符"
                )
        ...
        if len(self.replacement) > MAX_REPLACEMENT_LENGTH:
            raise RuleValidationError(
                f"替换内容不能超过 {MAX_REPLACEMENT_LENGTH} 个字符"
            )
        if self.action in {
...
class RuleImportConflictError(RuleConflictError):
    ...
    def __init__(
        ...
        names = "、".join(
            f"“{existing.name}”与“{imported.name}”"
            for existing, imported in self.conflicts[:3]
        )
...
class RuleLibrary:
    ...
    def from_dict(cls, value: Mapping[str, object]) -> RuleLibrary:
        ...
        rules = tuple(
            RuleDefinition.from_dict(_mapping(item, "rule"))
        ...
        *,
        ...
        for candidate in selected:
            ...
            if (
                ...
                raise RuleValidationError(
                    f"规则“{candidate.rule.name}”的结果仍保留完整原词"
                )
            matches.append(
                RuleMatch(
...
def default_standard_rules() -> tuple[RuleDefinition, ...]:
    ...
    specs = (
        ("手机号码", r"1[3-9]\d{9}", ActionKind.MASK_MIDDLE, "", "13812345678", "12345678901", 3, 4, "phone", False),
        ("身份证号", r"\d{17}[\dXx]", ActionKind.MASK_MIDDLE, "", "420101199003051234", "20260807123000001", 6, 4, "id_card", False),
        ("16 位银行卡号", r"\d{16}", ActionKind.MASK_MIDDLE, "", "6222021234567890", "202608071230000", 6, 4, "bank_card", False),
        ("标准车牌号", r"[京津沪渝冀豫云辽黑湘皖鲁新苏浙赣鄂桂甘晋蒙陕吉闽贵粤青藏川宁琼][A-Z][A-Z0-9]{5}", ActionKind.SEQUENCE_CODE, "车辆", "闽A12345", "CASEA12345", 0, 0, "vehicle_plate", False),
    )
    definitions: list[RuleDefinition] = []
    for index, (name, pattern, action, replacement, positive, negative, keep_prefix, keep_suffix, category, enabled) in enumerate(specs):
        definitions.append(
    ...
    manual_specs = (
        ...
        ("密码、口令与 PIN（内置识别后自动删除）", "SECRET", "密码：Abc#1234", "密码管理规范", "secret"),
        ("Token/API Key/AccessKey/SecretKey（内置识别后自动删除）", "SECRET", "token: eyJhbGciOi", "token 管理办法", "secret"),
    ...
    *,
    ...
    for rule in imported:
        ...
        same_source = [
            ...
            if item.source_signature() == source
        ]
        conflicting = [
            (index, item)
        ...
        if conflicting:
            if conflict_policy == "error":
                raise RuleImportConflictError(
                    tuple((item, rule) for _index, item in conflicting)
                )
...
def _select_candidates(candidates: Sequence[_Candidate]) -> list[_Candidate]:
    ...
    for candidate in ranked:
        ...
        if (
            ...
            continue
        raise RuleConflictError(
            f"规则“{candidate.rule.name}”与“{strongest.rule.name}”命中范围冲突"
        )
...
def _rule_from_import_row(row: Mapping[str, str], row_number: int) -> RuleDefinition:
    ...
    if match_mode_text not in {"仅人工", MatchMode.MANUAL.value} and not row.get(
        "关键词", ""
    ).strip():
        missing.append("关键词")
    if missing:
