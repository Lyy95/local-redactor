...
class LocalDesktopController:
    ...
    def scan(
        ...
        time_finding_ids = {
            ...
            if finding.category is Category.TIME
        }
        findings = [
            finding
            for finding in detected_findings
            ...
            and not (
                ...
                and time_finding_ids.intersection(
                    str(item)
                    for item in finding.metadata.get("contributor_ids", ())
                )
            )
        ]
        ...
        try:
            image_findings = [
                ...
                if finding.category is not Category.TIME
                and self._keep_image_finding(finding)
            ]
        except RuleLibraryError as exc:
            raise ValueError(
    ...
    def resolve_finding(
        ...
        if (
            status is FindingStatus.KEEP_FALSE_POSITIVE
        ...
        if (
            ...
            and any(
                item.original
                and contains_normalized_original(item.original, manual_replacement)
    ...
    def resolve_ordinary_findings(self) -> list[Finding]:
        ...
        combination_contributor_ids = {
            ...
            and risk.status is FindingStatus.PENDING
    ...
    def _related_findings(
        ...
        selected_rule_ids = {
            rule_id
            for rule_id in str(selected.metadata.get("rule_id", "")).split("；")
            if rule_id
        }
            ...
            for match in matches:
                ...
                if partially_overlapping:
                    for finding_id in dict.fromkeys(
                        item.id for item, _candidate in overlapping
                    ):
    ...
    def _apply_image_rules(self, findings: Sequence[Finding]) -> list[Finding]:
        ...
        for source in findings:
            ...
            status = (
                ...
                if not application.replacement
    ...
    def _rule_finding(
        ...
        *,
        ...
        if (
            match.action is not ActionKind.DELETE
            and contains_normalized_original(match.original, match.replacement)
        ):
            raise RuleLibraryError(
                f"规则“{match.rule_name}”替换后仍为原文，不能安全应用"
            )
        ...
        return Finding(
            ...
            confidence=1.0,
            suggested_method=(
                TransformMethod.REMOVE if remove else TransformMethod.ALIAS
            ),
    ...
    def _location_fully_covered(
        ...
        if (
            ...
            return False
        return (
            rule_location.start <= candidate.start
            and rule_location.end >= candidate.end
        )
    ...
    def _merge_rule_metadata(finding: Finding, match: RuleMatch) -> None:
        def merge_value(key: str, value: str) -> None:
            values = [
                item
                for item in str(finding.metadata.get(key, "")).split("；")
                if item
            ]
        ...
        finding.metadata["rule_mandatory"] = bool(
            finding.metadata.get("rule_mandatory")
        ) or match.mandatory
        ...
        for index, image in enumerate(document.images, start=1):
            ...
            rows.append(
                ImageReviewItem(
                    ...
                    original_summary=(
                        f"本机识别到 {len(image_findings)} 个人脸或敏感信息候选。"
                    ),
                    processed_summary=(
        ...
        *,
        ...
        rows = [
            ...
            ScanInventoryItem(
                ...
                len(document.images),
                "待人工决定" if any(
                    image.disposition is ImageDisposition.PENDING for image in document.images
