...
def _sanitize_relationship_part(data: bytes, name: str, removed_parts: set[str]) -> bytes:
    ...
    return cast(
        bytes, etree.tostring(root, xml_declaration=True, encoding="UTF-8", standalone=True)
    )
...
def _sanitize_content_types(
    ...
    existing_overrides = {
        node.get("PartName", "").lstrip("/"): node for node in root.findall(override_tag)
    }
...
def _sanitize_and_rewrite_word_part(
    ...
    if len(paragraphs) != len(block_ids):
        raise ExportSafetyError(f"Word 格式结构与扫描快照不一致（部件：{part}）")
    ...
    for tag, i1, i2, j1, j2 in matcher.get_opcodes():
        if tag == "equal":
            ...
            while cursor < i2:
                ...
                chunks.append((transformed[target_cursor : target_cursor + length], formatting))
                cursor += length
