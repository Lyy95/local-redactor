  ...
  return (
    <section className="history-panel" id="history-panel" aria-label="任务与历史">
      ...
      <div className="history-scroll">
        ...
        <div className="history-group compact-first">
          <h2>当前任务</h2>
...
export function App() {
  ...
  const [selectedSource, setSelectedSource] = useState(sampleSources[0]);
  const [mode, setMode] = useState("balanced");
  const [includeSubfolders, setIncludeSubfolders] = useState(true);
  const [boundaryAccepted, setBoundaryAccepted] = useState(true);
  const [workflowStep, setWorkflowStep] = useState(3);
  const [taskState, setTaskState] = useState("review_required");
  const [scanStage, setScanStage] = useState(scanStages.length - 1);
  const [findings, setFindings] = useState(initialFindings);
  const [resolutions, setResolutions] = useState({});
  const [replacements, setReplacements] = useState(() => Object.fromEntries(initialFindings.map((item) => [item.id, item.suggestion])));
  const [selectedFindingId, setSelectedFindingId] = useState(initialFindings[0].id);
  ...
  const currentTask = useMemo(() => ({
    id: "current",
    name: selectedSource?.name || "未命名任务",
    type: selectedSource?.type || "DOCX",
    status: currentStatus,
    tone: currentTone,
  }), [selectedSource, currentStatus, currentTone]);
  const folderSource = useMemo(() => sampleSources.find((source) => source.kind === "folder"), []);
  ...
  useEffect(() => {
    ...
    return () => window.clearTimeout(timer);
  }, [generationState, generationShouldFail, selectedSource, findings.length, batchSession?.currentId]);
  ...
  const startTask = (payload = selectedSource) => {
  ...
  const applySuggestion = (findingOrId) => {
  ...
  const keepOriginal = (findingOrId) => {
    const id = typeof findingOrId === "string" ? findingOrId : findingOrId?.id || selectedFindingId;
  ...
  const deleteFinding = (findingOrId) => {
    const id = typeof findingOrId === "string" ? findingOrId : findingOrId?.id || selectedFindingId;
  ...
  const addManualFinding = ({ original, suggestion }) => {
  ...
  const startGeneration = (forceFailure = false) => {
    if (pendingCount > 0) {
  ...
  const viewHistoryResult = (item) => {
    ...
    showToast("已打开本机结果摘要；原型不会打开真实文件");
  };
