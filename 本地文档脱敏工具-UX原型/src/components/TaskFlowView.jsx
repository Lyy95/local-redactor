...
function DocumentPreview({ findings, resolutions, replacements, selectedFindingId, onSelectFinding }) {
      ...
      <div className={`tf-review-layout ${manualMode ? "manual" : "quick"}`}>
        <div className="tf-document-area" ref={documentAreaRef}>
          <SuggestionCard
            ...
            actions={actions}
          />
          <DocumentPreview findings={findings} resolutions={resolutions} replacements={replacements} selectedFindingId={selectedFindingId} onSelectFinding={(id) => action(actions, "onSelectFinding", id)} />
        </div>
...
function GenerationProgress({ model }) {
  ...
  return (
    <div className="tf-generation-progress">
      <div className="tf-progress-heading"><div><strong>{state === "rescanning" ? "正在重新检查实际结果" : "正在生成示例结果"}</strong><span>{GENERATION_STAGES[activeIndex]?.detail}</span></div><span className="tf-progress-number">{progress}%</span></div>
  ...
  if (["generating", "rescanning"].includes(state)) {
    return (
      <section className="tf-page tf-generate-page" aria-labelledby="tf-generating-title">
        <header className="tf-page-heading"><span className="tf-heading-icon"><WandSparkles size={22} /></span><div><p className="tf-eyebrow">第 4 步</p><h2 id="tf-generating-title">{state === "rescanning" ? "重新检查实际结果" : "生成文件"}</h2><p>这是确定性的原型进度，不会写入真实文件。</p></div></header>
  ...
  return (
    <section className="tf-page tf-generate-page" aria-labelledby="tf-generate-title">
      ...
      <div className="tf-page-actions">
        ...
        <div className="tf-inline-actions">
          <button className="tf-secondary-button" type="button" onClick={() => action(actions, "onStartGeneration", true)}><AlertTriangle size={16} />模拟检查异常</button>
