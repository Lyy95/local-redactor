...
export default function NewTaskView({
  ...
  const validateAndStart = () => {
    if (!source) {
      const message = "请先选择一个虚构样例文件或文件夹。";
  ...
  return (
    <section className="new-task-view" aria-labelledby="new-task-title" data-testid="new-task-view">
      <header className="new-task-header compact">
        <div><p className="new-task-eyebrow">新建任务</p><h1 id="new-task-title">选择内容，开始检查</h1><p>使用虚构样例体验本机处理流程。</p></div>
      ...
      <div className="new-task-content compact">
        <section className="new-task-section quick-source" aria-labelledby="new-task-source-title">
          <div className="new-task-quick-head"><div><h2 id="new-task-source-title">检查来源</h2><p>选择单个文件或文件夹样例</p></div></div>
          <div className="new-task-source-actions compact">
            {Object.entries(sourceKindCopy).map(([kind, copy]) => {
              ...
              return <button key={kind} className={`new-task-source-button${source?.kind === kind ? " is-active" : ""}`} type="button" onClick={(event) => openChooser(kind, event.currentTarget)} aria-haspopup="dialog"><Icon size={19} /><span><strong>{copy.title}</strong></span></button>;
            })}
          </div>
          ...
          ) : <div className="new-task-empty-source"><Search size={21} /><div><strong>还没有选择内容</strong><span>请选择文件或文件夹样例。</span></div></div>}
        </section>
